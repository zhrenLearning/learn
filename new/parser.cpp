...
int OnMessageBeginCallback(http_parser *parser)
{
	...
	con->http_request_parser = std::make_shared<HttpRequest>();
	return 0;
}
...
{
	HttpConnection      *con = (HttpConnection*)parser->data;
	con->http_request_parser->http_headers[con->http_request_parser->http_header_field] = std::string(at, length);
	return 0;
}
...
{
	HttpConnection *con = (HttpConnection*)parser->data;
	con->http_request_parser->http_method = http_method_str((http_method)parser->method);
	return 0;
}
...
{
	HttpConnection *con = (HttpConnection*)parser->data;
	con->http_request_parser->http_header_field.clear();
	return 0;
}
...
int HttpParser::parserRequest(const char *buf, const std::size_t &bytes_transferred)
{
	int nparsed = http_parser_execute(&parser, &settings, buf, bytes_transferred);
	if (parser.http_errno != HPE_OK)
	{
		return -1;
	}
